# 下载

各位遇到非本软件的问题的时候,强烈建议自己先动手用搜索引擎或AI尝试解决,解决不了再问其他人,你遇到的大部分问题互联网均有记载

**若无法打开下载页面 [请修改电脑DNS](https://www.bilibili.com/video/BV1bY411z7u1/)**
**若无法打开下载页面 [请修改电脑DNS](https://www.bilibili.com/video/BV1bY411z7u1/)**

**若无法打开下载页面 尝试使用手机热点而非校园网试试**

<!-- 下载遇到问题?

**右键展开点保留**
<table>
  <tr>
    <td><img src="/qa_download.png" /></td>
    <td><img src="/qa_keep.png" height="245" /></td>
  </tr>
</table> -->


<!-- [推荐的解压软件](https://www.bandisoft.com/bandizip/dl.php?web) (没有好用的解压软件的可以下载这个,比较推荐,但非必选) -->

<!-- 请原谅我这么啰嗦, 如果我把下载放在最上面, 那么真的会有人在**答案就贴在下载的下面**的情况下, 告诉我**没看到**, 然后去群里问**为什么无法下载** -->

<EdgeDownloadLink href="https://msdzls.rainysnow.com">点我下载桌面版</EdgeDownloadLink> 提取码2333 (需先确定已关闭所有杀毒软件,**往下看完关于关闭杀毒软件的内容!!!往下看完关于关闭杀毒软件的内容!!!往下看完关于关闭杀毒软件的内容!!!往下看完关于关闭杀毒软件的内容!!!往下看完关于关闭杀毒软件的内容!!!**)

[开发版(可能不稳定, 新手勿选)](https://vip.rainysnow.com/)

[点我下载纯净Flash](https://rainy.lanzoul.com/ixPci1ictx6f) 提取码2333 (**不能使用Flash中心,必须先卸载Flash中心. 务必确保自己电脑里没有其他Flash后再安装这个!!!**)

<!-- (非必选)[点我下载游戏环境(Runtime)](https://rainy.lanzoul.com/iTsTi1lzg0qf) 提取码2333  (没提示缺少环境就不用下载) -->

[点我下载强卡器](https://rainy.lanzoul.com/b00efpw8vc) 提取码2333

## 双击安装包, 没有反应?

是因为你的系统设置启用了安全策略, 解决办法可以[参照这篇微软官方文章](https://learn.microsoft.com/zh-cn/answers/questions/5544639/question-5544639)

右键安装包, 点属性, 选择解除锁定就可以, 想永久关闭那个设置, 可以参考该链接下的官方回答.

## 务必关闭系统自带杀毒软件! 提示找不到指定dll文件的命令等等的解决方案

[添加软件到信任区 一劳永逸 点我查看操作视频(强烈建议,可以直接把C盘添加进去)](https://www.bilibili.com/video/BV1vz4y1U7Pm) **必须配置!!!必须配置!!!必须配置!!!必须配置!!!必须配置!!!必须配置!!!必须配置!!!必须配置!!!**

[想直接永久关闭杀毒软件? 点我查看操作视频](https://www.bilibili.com/video/BV15v4y1X7kw)

## 如何更新软件?

最推荐的方法是提前备份一份原软件文件夹(也可以不备份),然后下载最新的压缩包,压缩包解压覆盖到原来的文件夹即可,所有数据都在(除 自定义的公会任务)

你也可以手动打开 游戏窗口 文件夹, 复制里面所有的 .ini 后缀的文件,粘贴到新软件的 游戏窗口 文件夹下. 自定义战斗的数据在 游戏窗口 文件夹 下的 fight 文件夹下

## 提示： Windows 无法访问指定设备、路径或文件。你可能没有适当的权限访问该项目。

<img src="./img/windows_access_error.png" width=60%>

首先检查电脑是否有360, 金山毒霸这类杀毒软件, 如有, 请卸载后再尝试. 

如果仍然不行, 接着往下看

[解决办法](https://www.bilibili.com/video/BV1Ki4y157cV)

尝试视频和**评论区的所有方法**

在这个信息化的时代, 希望各位能学会用百度。根据提示信息到百度随便一搜都是解决方案。

## 常用链接

秒杀时间汇总 https://miaosha.rainysnow.com

桌面版BUG反馈收集 https://report.rainysnow.com

掉落物全地图查询 https://faa.msdzls.cn

美食数据站 https://msdzls.cn

卡片图鉴 https://tujian.rainysnow.com