...
/**
 * LucideIcon — 全站唯一的图标出口
 *
 * 只收录本站实际用到的 Lucide 图标（ISC License），内联几何数据，
 * 避免为几十个图标引入整包依赖。新增图标时把 Lucide 的内层元素原样贴进 ICONS 即可。
 *
 * 用法：<LucideIcon name="droplet" size="1.25rem" />
 */
...
const markup = computed(() => ICONS[props.name] || ICONS.droplet)
</script>
